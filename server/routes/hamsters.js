...
router.post('/add', async (req, res) => {
    try {
        ...
        const saveImage = await imgData.mv(__dirname + `/../public/uploads/${req.files.image.name}`,  (err) => {
            if (err) {console.log('Error saving file in uploadfolder:', err); throw err}
            console.log('fileimage saved in upload folder!')
        })
    
        const createStream = fs.createReadStream(__dirname + `/../public/uploads/${req.files.image.name}`)
        ...
        .then((data) => {
            fs.unlinkSync(__dirname + `/../public/uploads/${req.files.image.name}`, (err) => {
                if (err) throw err
            })
        })
